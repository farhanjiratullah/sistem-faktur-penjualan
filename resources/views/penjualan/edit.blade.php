...
@section('title', 'Edit Penjualan - Sistem Faktur Penjualan')
...
@section('content')
    <div class="p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Edit Faktur Penjualan</h1>
                <p class="text-gray-600 mt-1">Edit faktur penjualan: {{ $faktur->no_faktur }}</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <form action="{{ route('penjualan.update', $faktur->no_faktur) }}" method="POST" id="formPenjualan">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" value="{{ old('no_faktur', $faktur->no_faktur) }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 font-mono font-bold"
                                    readonly>
                                <input type="hidden" name="no_faktur" value="{{ old('no_faktur', $faktur->no_faktur) }}">
                                <input type="hidden" name="tgl_faktur" value="{{ $faktur->tgl_faktur }}">
                            </div>
                            ...
                            <div>
                                ...
                                <div id="perusahaan_customer"
                                    ...
                                    @php
                                        $selectedCustomerId = old('id_customer', $faktur->id_customer);
                                        $selectedCustomer = $customers->firstWhere('id_customer', $selectedCustomerId);
                                    @endphp
                                    {{ $selectedCustomer ? $selectedCustomer->perusahaan_cust : 'Pilih customer untuk melihat perusahaan' }}
                                </div>
                                @error('id_perusahaan')
                            ...
                            <div>
                                ...
                                <input type="date" name="due_date" id="due_date"
                                    value="{{ old('due_date', $faktur->due_date->format('Y-m-d')) }}" required
                        ...
                        <div class="space-y-4">
                                ...
                                <select name="id_customer" id="id_customer" required
                                    ...
                                    @foreach ($customers as $customer)
                                        <option value="{{ $customer->id_customer }}" data-alamat="{{ $customer->alamat }}"
                                            ...
                                            {{ (old('id_customer') ?? $faktur->id_customer) == $customer->id_customer ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="metode_bayar" id="metode_bayar" required
                                    ...
                                    <option value="TUNAI"
                                        {{ (old('metode_bayar') ?? $faktur->metode_bayar) == 'TUNAI' ? 'selected' : '' }}>
                                        TUNAI
                                    </option>
                                    <option value="TRANSFER"
                                        {{ (old('metode_bayar') ?? $faktur->metode_bayar) == 'TRANSFER' ? 'selected' : '' }}>
                                        TRANSFER</option>
                                    <option value="KREDIT"
                                        {{ (old('metode_bayar') ?? $faktur->metode_bayar) == 'KREDIT' ? 'selected' : '' }}>
                                        KREDIT
                                    </option>
                                </select>
                                @error('metode_bayar')
                    ...
                    <div class="border-t pt-6">
                        ...
                        <div class="space-y-4" id="produk-container">
                            ...
                            @php
                                ...
                                $existingProduk = $faktur->detailFaktur;
                            @endphp

                            @if ($oldProduk)
                                <!-- Jika ada old input, gunakan old input -->
                                @foreach ($oldProduk as $index => $produk)
                            ...
                            @else
                                ...
                                @foreach ($existingProduk as $index => $detail)
                                    <div class="produk-item border rounded-lg p-4 bg-gray-50">
                                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Produk</label>
                                                <select name="produk[{{ $index }}][id_produk]"
                                                    class="produk-select w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                    required>
                                                    <option value="">Pilih Produk</option>
                                                    @foreach ($produks as $p)
                                                        <option value="{{ $p->id_produk }}"
                                            ...
                                            <div>
                                                ...
                                                <input type="number" name="produk[{{ $index }}][qty]"
                                                    ...
                                                    class="qty-input w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                    required>
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Harga</label>
                                                <input type="number" name="produk[{{ $index }}][price]" readonly
                                                    value="{{ $detail->price }}" step="0.01" min="0"
                                                    class="price-input w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                    required>
                                            </div>
                                            <div class="flex items-end">
                                                <button type="button"
                                        ...
                                        <div class="mt-2 text-sm text-gray-600">
                                            Subtotal: <span class="subtotal-display font-semibold">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="number" name="ppn" id="ppn"
                                    value="{{ old('ppn', $faktur->ppn) }}" min="0" max="100" step="0.01"
                            ...
                            <div>
                                ...
                                <input type="number" name="dp" id="dp"
                                    value="{{ old('dp', $faktur->dp) }}" min="0" step="0.01"
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg">
                                <div class="flex justify-between items-center mb-2">
                                    ...
                                    <span id="subtotal" class="text-sm font-medium">
                                        Rp
                                        {{ number_format($faktur->grand_total + $faktur->dp - ($faktur->grand_total * $faktur->ppn) / 100, 0, ',', '.') }}
                                    </span>
                                </div>
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-sm font-medium text-gray-700">PPN:</span>
                                    <span id="ppn_value" class="text-sm font-medium">
                                        Rp {{ number_format(($faktur->grand_total * $faktur->ppn) / 100, 0, ',', '.') }}
                                    </span>
                                </div>
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-sm font-medium text-gray-700">DP:</span>
                                    <span id="dp_value" class="text-sm font-medium">
                                        Rp {{ number_format($faktur->dp, 0, ',', '.') }}
                                    </span>
                                </div>
                                <div class="flex justify-between items-center border-t pt-2">
                                    <span class="text-lg font-bold text-gray-800">Grand Total:</span>
                                    <span id="grand_total" class="text-lg font-bold text-blue-600">
                                        Rp {{ number_format($faktur->grand_total, 0, ',', '.') }}
                                    </span>
                                    <input type="hidden" name="grand_total" id="grand_total_input"
                                        value="{{ old('grand_total', $faktur->grand_total) }}">
                                </div>
                                @error('grand_total')
                                    <div class="mt-1 text-sm text-red-600 flex items-center">
                                        <i class="fas fa-exclamation-circle mr-2"></i>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Hitung produkCount berdasarkan data yang ada
            let produkCount = 0;
            const existingProdukItems = document.querySelectorAll('.produk-item');
            produkCount = existingProdukItems.length;
            ...
            // Add event listeners to existing produk items
            document.querySelectorAll('.produk-item').forEach(item => {
                ...
                // Update subtotal function for existing items
                function updateSubtotal() {
                    const qty = parseInt(qtyInput.value) || 0;
                    const price = parseFloat(priceInput.value) || 0;
                    const subtotal = qty * price;
